...
export default function MarketTitle({option, isOpen, setIsOpen, handleOptionClick}) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <OptionsList>
          <OptionItem
            onClick={() => handleOptionClick("가까운순")}
            style={{ borderBottom: "0.0625rem solid #C2C2C2" }}
          >
            <CheckIcon src={check} visible={option === "가까운순"} /> 가까운순
          </OptionItem>
          <OptionItem onClick={() => handleOptionClick("상품 많은 순")}>
            <CheckIcon src={check} visible={option === "상품 많은 순"} /> 상품
            많은 순
          </OptionItem>
        </OptionsList>
      )}
